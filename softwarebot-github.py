...
@bot.event
async def on_ready():
    activity = discord.Game(name="§help") #set bot status variable by §help
    await bot.change_presence(activity=activity) #show bot status
...
async def on_member_join(member):
       general_channel = client.get_channel(795333276868673560)
       general_channel.send(f"Bienvenue sur {guild.name} {member.display_name} !")

bot.run("xxxxxxxxxxxxxxxxxxxx") #token of the bot, you can find it on discord developer platform.
