...
async def auto_andwser(ctx, arg):
    await ctx.send(arg)
...
async def on_member_join(member):
       general_channel = client.get_channel(xxxxxxxxxx)
       general_channel.send(f"Bienvenue sur {guild.name} {member.display_name} !")

bot.run("xxxxxxxxxx") #token of the bot, you can find it on discord developer platform.
