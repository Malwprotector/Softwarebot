...
async def info(ctx):
    ...
    await ctx.send('**`SoftwareBot version 1.0 made by xxxxxxxxxx, coded in python3,  discord server xxxxxxxxxx | View code on github:`** https://github.com/Malwprotector/Softwarebot') #text that is sent by the bot
...
async def on_member_join(member):
       general_channel = client.get_channel(channel_number)  #you must insert instead of channel_number the number of the lounge where you want the bot to post, for that, activate discord in developer mode, make right-click on the desired lounge then click on copy the identifier.
       general_channel.send(f"Bienvenue sur {guild.name} {member.display_name} !")

bot.run("token") #token of the bot, you can find it on discord developer platform.